  ...
  },
  {
    ...
    actions: [
      ...
      },
      {
        ...
        color: '#ff5252',
        command: 'powershell',
        args: ['-NoProfile', '-Command', 'Get-CimInstance -Namespace root\\wmi -ClassName MSStorageDriver_FailurePredictStatus -ErrorAction SilentlyContinue | Select-Object InstanceName, PredictFailure, Reason | Format-Table -AutoSize; if (-not (Get-CimInstance -Namespace root\\wmi -ClassName MSStorageDriver_FailurePredictStatus -ErrorAction SilentlyContinue)) { Write-Output "SMART status not available on this system — physical drives may not expose WMI data" }'],
      },
      {
        id: 'god-mode',
    ...
    actions: [
      {
        ...
        color: '#00d4ff',
        command: 'powershell',
        args: ['-NoProfile', '-Command', 'ipconfig /flushdns; netsh winsock reset; netsh int ip reset; ipconfig /release; ipconfig /renew; Write-Output "Network reset complete. Restart your PC for changes to take full effect."'],
        confirm: 'This will flush DNS, reset Winsock, and reset TCP/IP. You may need to restart your PC for changes to take complete effect.',
        adminRequired: true,
      },
      {
        id: 'net-deep-reset',
      ...
      },
      {
        ...
        color: '#7986cb',
        command: 'powershell',
        args: ['-NoProfile', '-Command', 'if (Test-Path "$env:WINDIR\\System32\\msdt.exe") { Start-Process msdt.exe -ArgumentList "/id NetworkDiagnosticsWeb"; Start-Process msdt.exe -ArgumentList "/id DeviceDiagnostic"; Start-Process msdt.exe -ArgumentList "/id BluetoothDiagnostic" } else { Start-Process "ms-settings:network-status"; Start-Process "ms-settings:bluetooth"; Write-Output "msdt.exe not found (Windows 11 24H2+) — opening modern Settings pages instead" }; Write-Output "Done"'],
      },
      {
        id: 'update-trouble',
        ...
        color: '#00e676',
        command: 'powershell',
        args: ['-NoProfile', '-Command', 'if (Test-Path "$env:WINDIR\\System32\\msdt.exe") { Start-Process msdt.exe -ArgumentList "/id WindowsUpdateDiagnostic"; Start-Process msdt.exe -ArgumentList "/id BitsDiagnostic" } else { Start-Process "ms-settings:windowsupdate"; Write-Output "msdt.exe not found (Windows 11 24H2+) — opening Windows Update settings instead" }; Write-Output "Done"'],
      },
      {
        id: 'compat-trouble',
        ...
        color: '#ffab40',
        command: 'powershell',
        args: ['-NoProfile', '-Command', 'if (Test-Path "$env:WINDIR\\System32\\msdt.exe") { Start-Process msdt.exe -ArgumentList "/id PCWDiagnostic" } else { Start-Process "ms-settings:developers"; Write-Output "msdt.exe not found (Windows 11 24H2+) — opening Settings instead" }; Write-Output "Done"'],
      },
    ],
  },
  {
  ...
  },
  {
    ...
    actions: [
      ...
      },
      {
        ...
        color: '#e91e63',
        command: 'powershell',
        args: ['-NoProfile', '-Command', 'net stop spooler; Remove-Item -Path "$env:WINDIR\\System32\\spool\\PRINTERS\\*" -Force -ErrorAction SilentlyContinue; net start spooler; Get-Printer | Select-Object Name, DriverName, PortName, PrinterStatus, WorkOffline | Format-Table -AutoSize; if (Test-Path "$env:WINDIR\\System32\\msdt.exe") { Start-Process msdt.exe -ArgumentList "/id PrinterDiagnostic" } else { Start-Process "ms-settings:printers"; Write-Output "msdt.exe not found (Windows 11 24H2+) — opening Printer settings instead" }; Write-Output "Done"'],
        adminRequired: true,
      },
    ],
  },
